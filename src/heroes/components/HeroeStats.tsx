...
import { useHeroSumary } from '../hooks/useHeroSumarry'

export const HeroeStats = () => {
    const { data: summary } = useHeroSumary();
    return (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <HeroeStatCard title="Total Characters" icon={<Users className="h-4 w-4 text-muted-foreground" />}>
            ...
            {/** Heroe card */}
            <HeroeStatCard title="Favorites" icon={<Heart className="h-4 w-4 text-muted-foreground" />}>
                <div className="text-2xl font-bold text-red-600">3</div>
                <p className="text-xs text-muted-foreground">18.8% of total</p>
            </HeroeStatCard>

            {/* strongest hero */}
            <HeroeStatCard title="Strongest" icon={<Zap className="h-4 w-4 text-muted-foreground" />}>
                <div className="text-lg font-bold">{summary?.strongestHero.alias}</div>
                <p className="text-xs text-muted-foreground">Strength: {summary?.strongestHero.strength}/10</p>
            </HeroeStatCard>

            <HeroeStatCard title="Smartest" icon={<Trophy className="h-4 w-4 text-muted-foreground" />}>
