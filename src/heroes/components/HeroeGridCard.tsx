...
export const HeroeGridCard = ({ hero }: Props) => {
    ...
    return (
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-linear-to-br from-white to-gray-50">
            <div className="relative h-64">
                ...
                {/* Universe badge */}
                <Badge className={`absolute top-3 right-3 text-xs ${hero.universe === "DC" ? "bg-blue-500" : "bg-yellow-400"} text-white`}>
                    {hero.universe}
                </Badge>

                {/* Favorite button */}
                <Button size="sm" variant="ghost" className="absolute bottom-3 right-3 bg-white/90 hover:bg-white" onClick={() => toggleFavorite(hero)}>
                    <Heart className={`h-4 w-4 ${isFavorite(hero) ? 'fill-red-500 text-red-500' : 'text-gray-600'}`} />
                </Button>
                    ...
                    <div className="flex flex-wrap gap-1">
                        ...
                        </Badge> */}
                        {
                            hero.powers.slice(0, 3).map((power) => (
                                <Badge variant="outline" className="text-xs bg-gray-100" key={power}>
                                    {power}
                                </Badge>
                            ))
                        }
